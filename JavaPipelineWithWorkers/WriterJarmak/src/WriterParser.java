...
public class WriterParser {
    ...
    public static RC paramAnalysis(List<Lexem> lexems, WriterGrammar writerGrammar, Logger log){
        if (lexems == null || lexems.size() != writerGrammar.numberTokens() || lexems.isEmpty()){
            log.log(Level.WARNING, "null lexems or its empty or to much params");
            return RC.CODE_CONFIG_GRAMMAR_ERROR;
        }

        for (Lexem lex : lexems) {
            for (int i = 0; i < writerGrammar.numberTokens(); i++) {
                if (lex.getToken().equals(writerGrammar.token(i)))
                    break;
                else if (!lex.getToken().equals(writerGrammar.token(i)) && i == writerGrammar.numberTokens() - 1) {
